...
1. **Check for existing Claude comments**:
   ```bash
   gh api repos/{owner}/{repo}/issues/{pr_number}/comments --jq '.[] | select(.body | contains("<!-- claude-code-review -->")) | {id, created_at, user: .user.login}'
   ```

2. **Determine action**:
   - If existing Claude comment found AND it's the most recent comment → **UPDATE** it
   - If existing Claude comment found BUT newer comments exist → **CREATE NEW** (to avoid confusion)
   - If no existing Claude comment → **CREATE NEW**

3. **Execute action**:
   - **Update**: `gh api -X PATCH repos/{owner}/{repo}/issues/comments/{comment_id} -f body="{report}"`
   - **Create**: `gh pr comment {pr_number} --repo {owner}/{repo} --body "{report}"`

4. **Handle dry-run mode**:
   - If dry_run flag is true, skip GitHub API calls
   - Save report to `/tmp/pr-review-{pr_number}.md`
   - Log the file path for user review

### Step 7: Error Handling and Fallbacks
- **If gh CLI not available**: Save to `/tmp/pr-review-{pr_number}.md` and return file path
- **If API call fails**: Retry once, then save to temp file as fallback
- **If permissions insufficient**: Log error clearly and save to temp file
- **If PR number invalid**: Return error status with clear message
- **If input JSON malformed**: Return parsing error with details
