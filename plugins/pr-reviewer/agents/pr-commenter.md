...
**CRITICAL**: You have access to Bash, Read, and Write tools. You MUST actually use these tools, not just describe what commands you would run.

**Correct approach**:
1. Use Write tool to save report to file
2. Use Bash tool to execute gh CLI commands with --body-file flag
3. Actually invoke the tools - don't just show command examples

**Incorrect approach** ❌:
- Showing bash commands in markdown code blocks without using Bash tool
- Describing what you "would" do instead of doing it
- Trying to pass large multiline strings via command line arguments
...
_Generated by Claude Code PR Reviewer | {timestamp} | [Review Version {version}]_
```

### Step 6: Comment Management Strategy
Execute this decision tree using the actual tools available to you:

**CRITICAL**: You MUST use the Bash tool to execute commands. Do not just show what commands you would run - actually call the Bash tool.

1. **Write the report to a temp file first** (ALWAYS do this):
   Use the Write tool to save your formatted markdown report:
   ```
   Write tool: /tmp/pr-review-{pr_number}-{timestamp}.md
   Content: {your formatted markdown report}
   ```

2. **Check for existing Claude comments**:
   Use the Bash tool to execute:
   ```bash
   gh api repos/{owner}/{repo}/issues/{pr_number}/comments --jq '.[] | select(.body | contains("<!-- claude-code-review -->")) | .id' | head -1
   ```
...
4. **Execute action using Bash tool with --body-file**:

   **To CREATE new comment**:
   ```bash
   gh pr comment {pr_number} --repo {owner}/{repo} --body-file /tmp/pr-review-{pr_number}-{timestamp}.md
   ```

   **To UPDATE existing comment**:
   ```bash
   gh api -X PATCH repos/{owner}/{repo}/issues/comments/{comment_id} --input /tmp/pr-review-{pr_number}-{timestamp}.md -H "Content-Type: application/json" --jq '{body: input}'
   ```

   Or use gh pr comment edit if available:
   ```bash
   # First, add body field to temp JSON file
   echo '{"body": ' > /tmp/update-body.json
   jq -Rs '.' /tmp/pr-review-{pr_number}-{timestamp}.md >> /tmp/update-body.json
   echo '}' >> /tmp/update-body.json

   # Then update the comment
   gh api -X PATCH repos/{owner}/{repo}/issues/comments/{comment_id} --input /tmp/update-body.json
   ```

5. **Handle dry-run mode**:
   - If dry_run flag is true, skip GitHub API calls in step 4
   - Report saved to `/tmp/pr-review-{pr_number}-{timestamp}.md` is already done in step 1
   - Log the file path for user review: "Dry run mode: Review saved to /tmp/pr-review-{pr_number}-{timestamp}.md"

**IMPORTANT REMINDERS**:
- NEVER try to pass large multiline strings via --body flag
- ALWAYS use Write tool first to create the file
- ALWAYS use --body-file or --input with the file path
- ALWAYS actually invoke the Bash tool - do not just describe what you would do
...
- **If gh CLI not available**:
  - The report is already saved from Step 6.1 (Write tool)
  - Return the file path to the user: `/tmp/pr-review-{pr_number}-{timestamp}.md`
...
- **If API call fails**:
  ...
  - Message: "GitHub API call failed. Review saved to /tmp/pr-review-{pr_number}-{timestamp}.md"

- **If permissions insufficient**:
  - Report already saved, return file path
  - Message: "Insufficient permissions to post comment. Review saved to file."
...
```json
{
  ...
  "file_path": "/tmp/pr-review-{pr_number}.md",
  "summary": {
    "critical_count": 0,
    "important_count": 2,
    "suggestion_count": 5,
...
## Practical Example: How to Post a Comment

Here's exactly how you should execute the posting workflow:

**Step 1: Write report to file**
```
Tool: Write
file_path: /tmp/pr-review-123-20250212.md
content: <!-- claude-code-review -->
## Code Review Summary
...
```
Tool: Bash
command: gh api repos/owner/repo/issues/123/comments --jq '.[] | select(.body | contains("<!-- claude-code-review -->")) | .id' | head -1
description: Check for existing Claude review comment
```

**Step 3a: If no existing comment (create new)**
```
Tool: Bash
command: gh pr comment 123 --repo owner/repo --body-file /tmp/pr-review-123-20250212.md
description: Post new review comment to PR
```

**Step 3b: If existing comment found (update)**
```
Tool: Bash
command: jq -Rs '{body: .}' /tmp/pr-review-123-20250212.md | gh api -X PATCH repos/owner/repo/issues/comments/987654321 --input -
description: Update existing review comment
```

**DO NOT** try to do this:
```
❌ gh pr comment 123 --body "<!-- claude-code-review -->
## Code Review Summary
..."
```
This will fail with large comments and special characters.
